...
While most of these variables are self explanatory more information on them can be found in the Full Data documentation. Before consulting the full data documentation, the data dictionary, in prticular the tab called "final data_dictionary"", should be consulted to get a sense of what these variables are called in the dataset.
...
##Importation
I was interested in exploring the mentioned issue for the 2012/2013 academic year because it appeared to be the year with the most data available. Thus, I needed to import this dataset. 

Before beggining, install the follwing packages. 
...
```{r Main impotation, message = FALSE, warning = FALSE, tidy = TRUE}
...
* The dataset contained far more variables than my question required.
  + This would have make it difficult to understand how to analyze the data. 
...
CS2012b <- left_join(CS2012_13, CS2013_14, by = "OPEID") #Joined the two modified datasets using the OPEID variable, "OPEID is the identification number used by the U.S. Department of Education to identify institutions. This is a an 8-digit number that distinguishes between institutions, branches, additional locations, and other entities that are part of the eligible institution."
```
...
##Cleaning
###Cleaning Part 1
At this point the dataset has been adequately prepared, i.e. it contained the data I needed. Consequently, I began to clean the dataset for final analysis. Several problems were obvious and below I worked to address and mitigate these issues. 
...
However, to use these variables in a multiple linear regression they need to be converted into dummy variables. Fortunately, that can be done using the tidy package.
```{r Tidying, message = FALSE, warning = FALSE, tidy = TRUE}
...
CS2012 <- unique(CS2012) #Deleting duplicated rows to ensure that the spread function works
...
#Lastly, before beggining analysis variables were reordered, renamed, and consistent with proper methodology one dummy variable from each of the 4 sets above was removed and used as the baseline. For regions this was Far West, For locale this was Distant Town, For control this was Public, For predicted degree this was certificate.
CS2012 <- CS2012 %>% select(INSTNM, GREATLAKES = `Great Lakes`, MIDEAST = `Mid East`, NEWENGLAND = `New England`, PLAINS = Plains, ROCKYMOUNT = `Rocky Mountains`, SOUTHEAST = Southeast, SOUTHWEST = Southwest, FRINGETOWN = `Fringe Town`, LARGECITY = `Large City`, LARGESUB = `Large Suburb`, MIDSIZECITY = `Midsize City`, MIDSIZESUB = `Midsize Suburb`, REMOTETOWN = `Remote Town`, RURALDISTANT = `Rural Distant Territory`, RURALFRINGE = `Rural Fringe Territory`, RURALREMOTE = `Rural Remote Territory`, SMALLCITY = `Small City`, SMALLSUB = `Small Suburb`, PRIV4PROF = `Private for-profit`, PRIVNON = `Private nonprofit`, TUITFTE, INEXPFTE, AVGFACSAL, COSTT4_A, ASOCDEG = `Predominantly associate's-degree granting`, BACHDEG = `Predominantly bachelor's-degree granting`, SAT_AVG_ALL, PCIPSTEM, UGDS, UGDS_WHITE, UGDS_BLACK, UGDS_HISP, UGDS_ASIAN, UGDS_NRA, UGDS_WOMEN, PPTUG_EF, PFTFTUG1_EF, C150_4, LOAN_EVER, PELL_EVER, PCTPELL, RPY_1YR_RT, MD_EARN_WNE_P10) 
